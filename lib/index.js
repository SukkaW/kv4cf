...
const { MethodNotAllowedError, NotFoundError, InternalError } = require('./error');
...
const getAssetFromKV = async (event, options) => {
    ...
    const { request } = event;
    const { ASSET_NAMESPACE } = options;
    const ASSET_MANIFEST = typeof (options.ASSET_MANIFEST) === 'string' ? JSON.parse(options.ASSET_MANIFEST) : options.ASSET_MANIFEST;
    ...
    // Global caches Object -> Cloudflare
    const cache = caches.default
    let mimeType = getType(pathKey) || 'text/plain'
    if (mimeType.startsWith('text')) mimeType += '; charset=utf8';
    ...
    options.cacheControl = { ...defaultCacheControl, ...evalCacheOpts };
    ...
    let response = null;
    if (shouldEdgeCache) response = await cache.match(cacheKey);
    ...
    if (response) {
        ...
        if (body === null) throw new NotFoundError(`could not find ${pathKey} in your content namespace`);
